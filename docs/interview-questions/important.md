...
It helps you track, generate, and apply changes (migrations) to your database schema automatically.

A “Git” system for your database structure — tracking every schema change.
...
your_project/
│
├── alembic/                # Folder created by Alembic
│   ├── versions/           # Stores migration scripts
│   ├── env.py              # Main Alembic environment file
│   ├── script.py.mako      # Template for migration files
│
└── alembic.ini             # Configuration file (DB URL etc.)
````
...
Then open alembic/env.py and import your models’ metadata:
...
- Alembic scans your SQLAlchemy models (Base.metadata)
- Compares them with the actual database
- Generates a migration script (Python file) inside alembic/versions/

```py
# alembic/versions/3a2b4c_create_initial_tables.py
def upgrade():
    op.create_table(
        'users',
...
Alembic is a migration tool for SQLAlchemy that helps manage and version database schema changes.
It works by comparing your models with the current database, generating migration scripts in the alembic/versions/ folder, and applying them using commands like alembic upgrade head.
Each migration file has upgrade() and downgrade() functions to apply or revert changes safely.
- - -

## Q1. What is the difference between is operator & == operator in Python

- In general `is` operator meant for reference comparsion or address compression i.e if two reference are pointing to same object then only is operator returns True.
...
- Sometimes we can declare afunction without any name, such type of nameless functions are called anonymous functions or lambda functions.
